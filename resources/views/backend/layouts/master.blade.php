...
<head>
    ...
    <style>
        ...
        .panel-heading.note-toolbar.ui-sortable-handle {
            overflow: scroll;
        }
        .content {
            padding: 20px;
        ...
        span.select2.select2-container.select2-container--default.select2-container--above.select2-container--focus, span.select2.select2-container.select2-container--default.select2-container--above, span.select2.select2-container.select2-container--default {
            width: 100% !important;
        }
        .panel-heading.note-toolbar.ui-sortable-handle {
            overflow: hidden;
        ...
        $(document).ready(function() {
            ...
            $('#o_status').on('change', function() {
                if(confirm("Are you sure you want to change this order status?")){
                    ...
                    var order_id = $(this).parents('.gradeX').find('.order_id').val();
                    $.ajax({
                        url:"{{route('order.orderstatus.change')}}",
                        type:"POST",
                        data:{
                            "_token": "{{ csrf_token() }}",
            ...
            $('.user_status').on('change', function() {
                if(confirm("Are you sure you want to change this order status?")){
                    var user_status = this.value;
                    var user_id = $(this).parents('.gradeX').find('.user_id').val();
                    $.ajax({
                        url:"{{route('admin.profile.cs')}}",
                        type:"GET",
                        data:{
                            "_token": "{{ csrf_token() }}",
