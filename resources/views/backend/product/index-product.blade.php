                ...
                <div class="panel-body">
                    <table id="data-table-default" class="table table-striped table-bordered table-td-valign-middle">
                        ...
                        <tbody>
                            @foreach($data as $row)
                            @php
                            $category_name = App\Models\Category::where('id', $row->category_id)->value('category_name');
                            $brand_name = App\Models\Brand::where('id', $row->brand_id)->value('brand_name');
                            $product_image = App\ColorImage::where('product_id', $row->id)->orderBy('color_id')->take(1)->first();
                            @endphp
                            <tr class="odd gradeX">
                                <td>{{$loop->iteration}}</td>
                                <td><img width="80" src="@if($product_image){{asset($product_image->color_image)}} @else {{asset('defaults/noimage/no_img.jpg')}} @endif" alt="image"></td>
