...
<div id="detailpage">
	<div class="col-sm-12">
		<ul class="breadcrumb">
			...
			<li><a href="{{ url('/category',$product->category->slug) }}">{{$product->category->category_name}}</a></li>
					...
					<table width="100%" id="productcodeBox">
						<tr>
							...
							<td align="right">
								...
								<a href="javascript:;" class="favoritebtn">ADD TO FAVORITES </a>
							</td>
						</tr>
					</table>
										...
										<tr>
											<td valign="top" align="left" width="50%" nowrap="nowrap">
												<div class="imprintLocationItem">
													...
													@empty($cart[$product->id]['left_chest'])
													class="display-none"
													@endempty
													/>
													<select id="left_chest_no_of_imprint_color" name="left_chest_no_of_imprint_color" class="snum">
	...
	<div id="liveChat" style="display: none;">
		...
		<div class="card-body" style="padding: 15px;">
			...
			<div style="margin-top: 15px;">
				...
				<input type="text" aria-label="Email Address" id="email" name="email" style="height: 30px; width: 100%;">
			</div>
			<div style="margin-top: 15px;">
				<span>Phone Number *</span> 
				<input type="text" aria-label="Phone" id="mobile" name="mobile" style="height: 30px; width: 100%;">
			</div>
			<div style="margin-top: 15px;">
				<span>Question *</span> 
				<input type="text" aria-label="question" id="questions" name="questions" style="height: 30px; width: 100%;">
			</div>
...
<div id="emailUsForm">
	...
	<form id="mail_us">
		...
		<div style="margin-top: 15px;">
			...
			<input type="text" aria-label='Mobile Number' id="mobile" name="mobile" style="height: 30px; width: 100%;" />
		</div>
		<div style="margin-top: 15px;">
		...
		<input type="hidden" name="product_id" id="product_id" value="{{$product->id}}" />
		<input type="submit" class="aemailus btn" value="Submit" />
		<a href="javascript:$('#emailUsForm').hide()">Cancel</a>
	</form>
	<hr>
	<p class="help">Helpful Links</p>
	<a href="{{route('aboutus')}}" style="display: block; text-decoration: underline !important;">Contact Us</a> 
	<a href="{{route('contactinfo')}}" style="display: block; text-decoration: underline !important;">FAQs</a>
</div>
</div>
<div class="col-sm-12">
	<div id="botwrap-product">
			@php
			$similar_products = App\Models\Product::where('category_id', $product->category_id)->take('5')->get();
			@endphp
			
		<div class="titlebg">You may also like</div>
		<div class="productwrap">
			@foreach($similar_products as $pp)
			@php
			$product_image = App\ColorImage::where('product_id', $pp->id)->orderBy('color_id')->take(1)->first();
			@endphp
			<div class="box">
				<div class="thumbnails"><a href="{{url('product', $pp->product_slug)}}">
					<img src="@if($product_image) {{asset($product_image->color_image)}} @else {{asset('defaults/noimage/no_img.jpg')}} @endif" class="img-responsive" alt="16696 - Sport-Tek® V-Neck Raglan Wind Shirt">
				</a>
			</div>
			<div class="catprodetail">
				<div class="caption">
					<p>{{$pp->product_code}}</p>
					<a href="{{url('product', $pp->product_slug)}}">
						<h2>{{$pp->product_name}}</h2>
					</a>
					<span>As low as: {{$pp->selling_price}}</span>
				</div>
			</div>
		</div>
		@endforeach
